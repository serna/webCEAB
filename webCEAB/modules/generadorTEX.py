...
def toLatexQuestion(question,order,linea):
	...
	ans += "\n\t\\end{enumerate}"
	return ans
def latexPreamble(testName,clave,version,nPreguntas,claveAlumno2,claveMateria):
...
\\author{"""
	...
	ans += "\\\\Total de preguntas: " + str(nPreguntas) + "\\\\Alumno: " + str(claveAlumno2) + "\\\\Materia: " + str(claveMateria)
	ans += """\\\\} % Your name
...
def crea_archivo(nombre,contenido):
	...
	materia = contenido["nombreBanco"]
	nombreAlumno = contenido["alumno"]
	claveMateria = contenido["claveMateria"]
	preguntas_validas = []
	...
	cadena = genera_TEX_PDF(folio_alumno=folio,en_orden = en_orden,listaPreguntas=preguntas_validas,testName = materia,nombreAlumno=nombreAlumno,claveMateria=claveMateria)
	#print(cadena)
	ff = open(nombre,"w")
	ff.write(cadena)
	ff.close()
...
def genera_TEX_PDF(folio_alumno,listaPreguntas,testName,nombreAlumno,claveMateria,en_orden = 0):
	...
	#ff1 = open(fileName,"w")
	cadena = latexPreamble(testName,clave,versionStr,len(listaPreguntas),nombreAlumno,claveMateria) # start writing latex file
